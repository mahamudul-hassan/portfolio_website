...
(function initChatbot() {
  ...
  toggle.addEventListener('click', () => {
    if (isOpen) {
      ...
      openPanel();
      // Show chips on first open alongside the greeting already in the DOM
      if (!chipsShown) addQuickChips();
    }
  });
  closeBtn.addEventListener('click', closePanel);

  // Render markdown-like bold (**text**) and bullets into HTML
  function formatBotText(text) {
    ...
    // Bullet lines starting with • or - (at start of line)
    ...
    // Links: convert plain https:// URLs to clickable links
    html = html.replace(
      /(https?:\/\/[^\s<)"]+)/g,
  ...
  // Quick-reply chips shown once after the first bot greeting
  const CHIPS = [
    'What are your research interests?',
    'Tell me about your publications',
    'Are you open to PhD positions?',
  ...
  function addQuickChips() {
    ...
    CHIPS.forEach(label => {
      ...
      chip.addEventListener('click', () => {
        // Remove all chips immediately on click
        const chipWrapper = document.getElementById('quick-chips');
        if (chipWrapper) chipWrapper.remove();
  ...
  async function sendMessage() {
    ...
    try {
      ...
      // Show quick-reply chips after the very first bot response
